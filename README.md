...
- UI Toolkit 固定玩家文案在 UXML 中直接声明 Localization binding；真实动态文本也必须 author 非空设计期值并声明运行时 owner，任何空 `Label` 都不能以运行时覆盖为由通过审查。
- UI Toolkit 图标由独立 `VisualElement` 承担，禁止把图标作为 `Label` 的背景图与文本拼接。
