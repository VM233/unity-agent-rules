...
- 实现、修复或内容修改包含直接相关、风险匹配的验证，不需要用户单独授权或点名工具。Unity 代码或编译契约变化必须完成权威编译并处理全部 error；其他改动先做静态及领域审查，再按行为、视觉、交互或打包风险补足验证。涉及跨战斗生命周期且产品支持重新战斗/重试时，受影响的 Toolbar、场景或运行时测试必须重新产生第二次战斗证据，不能只证明首次运行。用户明确限制、数据安全、业务提交与产品发布边界保持有效。
