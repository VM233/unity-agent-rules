...
- 机制事实表与玩家描述不是同一份清单：实现和测试完整覆盖合法状态，UI 只保留正常游玩中影响当前决策的触发、目标、效果和必要例外。不得为了证明代码支持某种情况，就把内部参数、推导过程或低频边界全部写进默认描述。
...
- 审查器验证实际选用文本的完整性、参数绑定、真实数值和各 Locale 一致性，不能反向强制每个内部 Count consumer/变量都出现在描述里。写入前逐句判断“删掉是否影响正常玩家理解或选择”，不影响的叠加解释、重复上下文和实现细节删除；这一判断不得用逐内容白名单或特殊豁免代替
