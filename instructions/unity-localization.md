...
- 机制事实表、实现/测试契约与 UI 展示稿是不同产物。“完整覆盖”“不得漏配”“必须验证”约束功能、配置和证据，不要求 UI 逐项复述；默认描述只保留正常游玩中影响选择或操作的核心效果、关键条件和必要例外。不得把事实表、组件清单或验收步骤直接改写成玩家说明。
...
- 审查器验证实际选入文本的语义、参数绑定、真实数值和各 Locale 一致性，不强制暴露每个效果分支、内部变量或 Count consumer。写入前逐句判断“删掉是否影响正常玩家理解、选择或操作”，不影响的叠加解释、重复上下文和实现细节删除；不得靠逐内容白名单或特殊豁免代替这一判断。
