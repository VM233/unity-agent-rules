...
- 修改 package metadata、manifest、lockfile 或 Unity 可见文件时只做定向编辑，并在当前 checkout 重新调用本文件的 `package/dependency-policy-review`；不得依赖旧工具响应、旧端口或旧解析缓存。
...
- README 只保留首次采用前后必须稳定获知的入口信息：package/plugin 的用途与边界、package ID、不可变安装方式、已声明支持范围与要求、依赖权威、顶层目录、最小接入/验证入口，以及 CHANGELOG、许可和专门文档的链接。超出这一职责的信息不得以“完整”“方便查阅”或历史留档为由留在 README。
- README 严禁承载版本特定行为或修复史、迁移步骤、内部实现与算法解释、详细 API 示例或教程、功能专属 Editor 操作手册，以及完整命令、route、schema、测试矩阵或按开发时间累积的说明；README 不得兼任 CHANGELOG、迁移手册、API reference、Editor 手册或测试目录。唯一允许的示例是完成安装或首次接入所需的最小且稳定片段。
