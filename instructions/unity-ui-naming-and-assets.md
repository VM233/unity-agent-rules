...
- 除非用户明确要求改变主体大小或构图，图片缩放只对原始完整画布做等比缩放，保留透明留白、构图位置和主体相对画布的占比。调整输出像素尺寸不授权单独放大或缩小主体；这里的主体大小指其在整图中的相对大小，而非缩放前后的绝对像素数。
- 禁止默认裁掉透明边缘、按非透明包围盒重新取景、调整主体后重新居中，或为了填满 UI、对齐同类图标视觉大小而统一主体占比。“注意缩放”“重新导入”和指定输出宽高都不构成这些操作的授权。
- 替换或重新导入时以用户批准的原图及其完整留白为基准，不能把此前擅自裁剪或放大主体的版本作为缩放源。交付前对照原图核对主体占比与位置，区分整图分辨率变化和主体构图变化。
